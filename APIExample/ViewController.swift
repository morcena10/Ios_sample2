...
import Floaty
import AgoraRtcKit
import CoreBluetooth  // For Bluetooth usage
import EventKit  // For Calendar and Reminders
import AVFoundation  // For Camera and Microphone
import Contacts  // For Contacts usage
import MobileCoreServices  // For handling file types and documents picker
import HealthKit  // For Health data access
import CoreLocation  // For Location Services
import Photos  // For Photos and Photo Library
import Speech  // For Speech Recognition
import ReplayKit  // For Screen Capture (Recording Screen)
...
//            MenuItem(name: "Raw Media Data".localized, storyboard: "RawMediaData", controller: "RawMediaData"),
    ...
    override func viewDidLoad() {
        ...
        AVAudioSession.sharedInstance().requestRecordPermission { (granted) in
               ...
               // Bluetooth permissions are handled automatically when connecting to peripherals using CoreBluetooth

               // Request HealthKit Permissions (For Health Data)
               ...
               reminderStore.requestAccess(to: .reminder, completion: { (granted, error) in
                   if granted {
               ...
               // Request Screen Recording (System Feature)
               if #available(iOS 11.0, *) {
                   UIScreen.main.isCaptured // Check if screen capture is enabled
               }
